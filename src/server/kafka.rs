...
pub(crate) fn consume_via<F>(topic: &str, mut message_handler: F)
where
    F: FnMut(PrimaryKey, Value) -> (),
{
    println!("Topic: {}", topic);
    ...
    loop {
        ...
        messages
            ...
            .map(|message| {
                (
                    Uuid::from_slice(message.key).unwrap(),
                    Reader::new(message.value).unwrap(),
                )
            })
            .flat_map(|(key, value_list)| value_list.map(move |value| (key, value.unwrap())))
        ...
        let (updated_commit, updated_messages) =
            commit_offset_if_needed(&mut consumer, &latest_commit, consumed_messages);
...
fn commit_offset_if_needed(
    ...
    consumed_messages: bool,
) -> (Instant, bool) {
    let current_instant = Instant::now();
    if current_instant.gt(&latest_commit.add(Duration::from_secs(30))) && consumed_messages {
